...
Data_Matrix = ones(Photo_counter,16)*NaN;
% This matrix will store the values of the corresponding image in the
% following order:
% Longitud - Latiud - Altitud
% r_planta - g_planta - n_planta
% - SR - NDVI - GNDVI - TVI - CTVI - SAVI - DVI
% - MSAVI - Biomass - Nitrogen


for id_photo = 1:Photo_counter
    ...
    Photo_Name = Complete_Data{id_photo,1};
    Photo_Name = Photo_Name(1:22);
    photo_number = Photo_Name(5:end);

    Data_Matrix(id_photo,1) = str2double(Complete_Data{id_photo,3});
    Data_Matrix(id_photo,2) = str2double(Complete_Data{id_photo,4});
    Data_Matrix(id_photo,3) = str2double(Complete_Data{id_photo,5});

    eval(['Photo_Path = Path_' num2str(Complete_Data{id_photo,2}) ';']) % Path to the photo
    img_GRE = imread([Photo_Path 'Fotos_Procesadas/' Photo_Name '_GRE.TIF']);
    img_NIR = imread([Photo_Path 'Fotos_Procesadas/' Photo_Name '_NIR.TIF']);
    img_RED = imread([Photo_Path 'Fotos_Procesadas/' Photo_Name '_RED.TIF']);
    img_REG = imread([Photo_Path 'Fotos_Procesadas/' Photo_Name '_REG.TIF']);
    img_RGB = imread([Photo_Path 'Fotos_Procesadas/' Photo_Name '_RGB.JPG']);
    ...
    [num_planta, sel_planta] = max([num_ctr_1, num_ctr_2]);
    ...
    % Las coordenadas del centro de la no planta
    r_no_planta = ctrs(sel_no_planta,1);
    g_no_planta = ctrs(sel_no_planta,2);
    n_no_planta = ctrs(sel_no_planta,3);
    
    % Mirando las anteriores para quitar los errores.
%     if (id_photo>10)
%         
%         mean_sel_r = mean(Data_Matrix((id_photo-10):(id_photo-1), 4));
%         mean_sel_g = mean(Data_Matrix((id_photo-10):(id_photo-1), 5));
%         mean_sel_n = mean(Data_Matrix((id_photo-10):(id_photo-1), 6));
%         
%         dist_planta = sqrt((r_planta - mean_sel_r)^2 + (g_planta - g_no_planta)^2 + (n_planta - n_no_planta)^2);
%         dist_no_planta = sqrt((r_no_planta - mean_sel_r)^2 + (g_no_planta - g_no_planta)^2 + (n_no_planta - n_no_planta)^2);
%         
%         if dist_no_planta<dist_planta
%             
%             r_planta = r_no_planta;
%             g_planta = g_no_planta;
%             n_planta = n_no_planta;
%             num_no_planta = num_planta;
%             sel_no_planta = sel_planta;
%             
%         end
%         
%     end
    ...
    Data_Matrix(id_photo, 4) = r_planta;
    Data_Matrix(id_photo, 5) = g_planta;
    Data_Matrix(id_photo, 6) = n_planta;
    ...
    Measurement = Complete_Data{id_photo,7};
    % 'Measurement' is a vector with x-position, y-position, IR64 materia 
    % fresca and IR64 materia seca, SPAD IR64 around the nearest point  
    Photo_Position = Complete_Data{id_photo,6};
    % 'Photo_Position' is a vector with x-position, y-positon, z-position of the photo
    
    radius = 250 + 30*randn(1);
    
    % Select only the region near the measured waypoint.
    Weigths = Position_in_Photo(Measurement, Photo_Position, Weigths, radius);
    ...
    % Save images
    saveas(Char_Space_2D, [Photo_Path 'Resultados/'  'Char_Space_2D_' num2str(photo_number) ] ,'jpg')
    saveas(Char_Space_3D, [Photo_Path 'Resultados/'  'Char_Space_3D_' num2str(photo_number) ] ,'jpg')
    saveas(Img_Weigths, [Photo_Path 'Resultados/'  'Im_Weigths_' num2str(photo_number) ] ,'jpg')
    saveas(Img_Selec, [Photo_Path 'Resultados/'  'Im_Selec_' num2str(photo_number) ] ,'jpg')
    saveas(Final_Img, [Photo_Path 'Resultados/'  'Final_Img_' num2str(photo_number) ] ,'jpg')
