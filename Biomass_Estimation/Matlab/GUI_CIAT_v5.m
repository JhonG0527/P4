...
Ob19 = uipanel(Ob14,'Title','Groud truth measurements','FontSize', Font_size_18,'Units','normalized','Position',Ob19_pos);
    ...
    function [] = callb(H,E)
            ...
            if (existe_gre*existe_nir*existe_red*existe_reg)>0 % Si todos los archivos existen
                ...
                hay_info_GPS = 0; % no hay informacion GPS
                        ...
                        if not(hay_info_GPS) % Si al final no hubo informacion GPS
                            Lista_Nombres{cont_fotos,3} = 'nan'; % Latitud
                            Lista_Nombres{cont_fotos,4} = 'nan'; % Longitud
                            Lista_Nombres{cont_fotos,5} = 'nan'; % Altura      
                        end
       ...
       % la siguiente manera: Longitude - Latitude - Biomass - SPAD
        ...
        for id_Home_dir = 1:cont_Home_dir % Recorrer todos los directorios
            ...
            MyFolderInfo = dir([ Path_to_Folder backslash Lista_Home_dir{id_Home_dir}  backslash '*.mat']);
...
% Funcion llamada por la tabla de las medidas de biomasa
    function table_Callback(hObject,callbackdata)
        ...
        % Ahora es necesario actualizar la matriz en el .mat original
            ...
            % Buscar dentro de la carpeta apropiada el archvio .mat, solo deberia haber un archivo
            MyFolderInfo = dir([ Path_to_Folder backslash Lista_Home_dir{Op_Selec_Home_dir(length(Op_Selec_Home_dir))}  backslash '*.mat']);
...
% Funcion llamada por el boton de inicio
    ...
    function Startbutton_Callback(source,eventdata) 
        ...
        for id_Photos_Proc = 1:length(ID_Photos_Procesar) % Recorrer todas las fotos que se van a procesar
            ...
            Ob22.String = ['Image ' num2str(id_Photos_Proc) backslash num2str(sum(Num_Photos_in_dir(Op_Selec_Home_dir,2))) ' Name: '  Lista_Nombres{ID_Photos_Procesar(id_Photos_Proc),1}]; % Actualizar el numero de la imagen que se lleva y el nombre
        ...
        save('Matriz_Total.mat', 'Photo_Data')
        
        % Quitar las fotos con nan en la biomasa y el spad
        
        Photo_Data=Photo_Data(find(not(isnan(Photo_Data(:,9))).*not(isnan(Photo_Data(:,10)))),:);
        
        % 1.3.) De todas estas se toma un subconjunto al azar para el entranamiento
        random_ids = rand(1,length(Photo_Data(:,1)))>(1-(Training_Percentage/100));
